...
def patientLevelDataSplitter(excel_path,save_path,train_split=0.85,tolerance=0.025):
    ...
    while scansInVal<total_volumes*(1-train_split):
        ...
        scansInVal+=frequencies[sampled[0]]
        patients.remove(sampled[0])
        val_indices.extend(df.index[df['research_id']==sampled[0]].tolist())
    ...
    assert len(train_indices) <=(train_split+tolerance)*total_volumes and len(train_indices) >= (train_split-tolerance)*total_volumes, f' the split has an deviation more than permissible tolerance \n permissible tolerance:{tolerance} \n deviation:{abs(len(train_indices)-total_volumes*train_split)/total_volumes}'
    
    
    with open(save_path,'w') as f:
        json.dump({'train_indices':train_indices
                     ,'val_indices':val_indices},f,indent=4)

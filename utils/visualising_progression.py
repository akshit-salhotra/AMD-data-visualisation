...
for patient in tqdm(patients):
    ...
    while i<len(all_paths):
        if f'{patient+os.sep}L' in all_paths[i]:
            progression[patient]['L'].extend([get_amdtype(patient_info_from_path(all_paths[i]),dataframe)])
            i+=1
        else:
            break
    ...
    while i<len(all_paths):
        if f'{patient+os.sep}R' in all_paths[i]:
            progression[patient]['R'].extend([get_amdtype(patient_info_from_path(all_paths[i]),dataframe)])
            i+=1
        else:
            break

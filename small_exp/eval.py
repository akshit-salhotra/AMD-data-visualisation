...
def evaluate_dataset(json_path:str,device:torch.device,model_path:str,excel_path:str,transform,batch_size:int,num_workers:int,timeout:int,save_path:str)->None:
    ...
    with torch.no_grad():
        for image,label in dataloader:
            image=image[0].to(device)#changed due to collate functions
